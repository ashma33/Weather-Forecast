...
function renderTemperatures() {
    ...
    if (elements.feelsLike) elements.feelsLike.textContent = `${Math.round(feels)}${isCelsius ? '°C' : '°F'}`;
...
function renderForecastTracks() {
    ...
    // unit label — C or F depending on current toggle state
    ...
    currentForecast.list.slice(0, 8).forEach(item => {
        ...
        elements.hourlyForecast.innerHTML += `
            <div class="hourly-item">
                ...
                <div class="hourly-item-temp">${temp}${unitLabel}</div>
            </div>`;
    });
            ...
            elements.dailyForecast.innerHTML += `
                <div class="daily-item">
                    ...
                    <div class="daily-item-temps">
                        <span class="daily-item-high">${tempHigh}${unitLabel}</span>
                        <span class="daily-item-low">${tempLow}${unitLabel}</span>
                    </div>
                </div>`;
        }
    });
